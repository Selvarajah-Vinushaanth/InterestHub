...
# Initialize Supabase client - fix the token format
SUPABASE_URL = "https://kfmvscncaombppcmlqjo.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtmbXZzY25jYW9tYnBwY21scWpvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDMzMzM0NjgsImV4cCI6MjA1ODkwOTQ2OH0.ex_R_PRFwKhUOu8SAn8cErXWanZ_KOp7vfLraPU-MGE"
            ...
            if file_type == "video":
                try:
                    ...
                    test_response = requests.post(
                        ...
                        headers={
                            "apikey": SUPABASE_KEY,
                            "Authorization": f"Bearer {SUPABASE_KEY}",  # Fixed extra comma
                            "Content-Type": "application/json",
...
def get_messages(group):
    try:
        print(f"Fetching messages for group: {group}")
        ...
        if response.status_code != 200:
            print(f"Error response from Supabase: {response.text}")
...
def get_message_reactions(message_id):
    try:
        ...
        print(f"Error retrieving reactions: {str(e)}")  # Fixed missing closing quote
...
def test_supabase():
    try:
        ...
        print(f"Test response body: {test_response.text}")
        
        if test_response.status_code == 200:
            return jsonify({"success": True, "message": "Supabase connection successful"}), 200
        else:
            return jsonify({
        ...
        return jsonify({"error": "Test failed with exception", "details": str(e)}), 500
